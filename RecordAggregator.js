...
// constructor
function RecordAggregator(records, maxKinesisPayloadSize) {
	if (!AggregatedRecord) {
		AggregatedRecord = common.loadBuilder();
	}
	...
	if(maxKinesisPayloadSize) {
		this.maxKinesisPayloadSize = Math.min(maxKinesisPayloadSize, KINESIS_MAX_PAYLOAD_BYTES);
	} else {
        this.maxKinesisPayloadSize = KINESIS_MAX_PAYLOAD_BYTES;
	}
...
RecordAggregator.prototype.flushBufferedRecords = function(onReadyCallback) {
	if(debug) { console.log("calculated totalBytes=" + this.totalBytes); }
